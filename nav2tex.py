...
class Nav2Tex(nn.Module):
    ...
    def generate(self, images, tokenizer, max_new_tokens: int = 512, device="cpu", encoder_key_mask=None):
        ...
        if encoder_key_mask is None and images.shape[0] > 1:
            patch_size = getattr(self, "_patch_size", 16)
            ph = images.shape[2] // patch_size
            pw = images.shape[3] // patch_size
            encoder_key_mask = torch.ones(images.shape[0], ph * pw, dtype=torch.bool, device=device)
        elif encoder_key_mask is not None:
        ...
        for _ in range(max_new_tokens):
            logits = self.decoder(input_ids, encoder_output=encoder_output, encoder_key_mask=encoder_key_mask)
